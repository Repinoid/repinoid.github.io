...
sortByTests() ;
	...
	for ( let i=0 ; i < jon.length; i++ ) {
			jon[i].region = jon[i].region.replace('область', "").trim() ;
			jon[i].region = jon[i].region.replace('край', "").trim();
			jon[i].region = jon[i].region.replace('Республика', "").trim();
			jon[i].region = jon[i].region.replace('республика', "").trim();
			jon[i].region = jon[i].region.replace('автономная', "").trim();
			jon[i].region = jon[i].region.replace('автономный округ', "").trim() ;
			jon[i].region = jon[i].region.substring(0,15) ;
	}
...
//					check.type = "checkbox"; 
			...
			const bgc = "font: 12px arial; font-weight: 600; background-color: rgba(0,0,255,"  + (0.5 - i * step) + "); " ;
